...
log_dir = './logs'
model = DQN(policy="CnnPolicy", env=env, verbose=1,tensorboard_log=log_dir)

# saves every TIMESTEPS steps
TIMESTEPS = 10000
for i in range(1,30):
    model.learn(total_timesteps=TIMESTEPS,reset_num_timesteps=False, tb_log_name="DQN", device='cuda')
