...
class QNetwork:
    ...
    def act(self, possible_states):
        ...
        max_rating = None
        best_state = None
        ratings = self._predict_ratings([state[2:] for state in possible_states])

        for i, sa in enumerate(possible_states):
            action = sa[:2]
            state = sa[2:]
            rating = ratings[i]
    ...
    def learn(self, batch_size=512, epochs=1):
        ...
        # batch = [x for x in batch if isinstance(x, (np.ndarray, np.generic)) and len(x) == 9]
